...
class VarsExtractor:
    ...
    def vars_replace(self, target: dict, env_filename: str | None = None) -> dict:
        ...
        try:
            env_file = os.path.join(httpfpt_path_config.run_env_dir, env)
            env_vars = get_env_dict(env_file)
        except OSError:
            raise RequestDataParseError('运行环境获取失败, 请检查测试用例环境配置')

        # 获取全局变量
        global_vars = read_yaml(httpfpt_path_config.data_path, filename='global_vars.yaml')
    ...
    def teardown_var_extract(response: dict, extract: dict, env: str) -> None:
        ...
        if set_type == VarType.CACHE:
            ...
            write_env_vars(httpfpt_path_config.run_env_dir, env, key, value_str)
